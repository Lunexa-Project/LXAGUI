...
Lunexa is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both Lunexa and Bitcoin donations can be made to **donate.getmonero.org** if using a client that supports the [OpenAlias](https://openalias.org) standard.
...
* Flatpak: [Lunexa GUI](https://flathub.org/apps/details/org.getmonero.Lunexa)
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/lunexa-gui -w /lunexa-gui lunexa:build-env-windows sh -c 'make depends root=/depends target=x86_64-w64-mingw32 tag=win-x64 -j4'
   ```
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/lunexa-gui -w /lunexa-gui lunexa:build-env-linux sh -c 'make release-static -j4'
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `lunexa-gui` directory  
   \* `4` - number of CPU threads to use
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/lunexa-gui -e THREADS=4 lunexa:build-env-android
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `lunexa-gui` directory  
   \* `4` - number of CPU threads to use
